...
class HandleSeoRedirects
{
    ...
    /**
     * @param  Builder<SeoRedirect>  $baseQuery
     */
    protected function findPatternRedirect(Request $request, Builder $baseQuery): ?SeoRedirect
    {
     ...
     */
    protected function patternRedirects(Builder $baseQuery): Collection
    {
        $cacheSeconds = (int) config('lazy-seo-redirects.cache_seconds', 60);

        $callback = fn (): Collection => (clone $baseQuery)
            ->pattern()
            ->orderByDesc('id')
            ->get();
        ...
        $rows = Cache::remember(
            'lazy-seo-redirects.patterns',
            $cacheSeconds,
            fn (): array => $callback()
                ->map(fn (SeoRedirect $redirect): array => $redirect->getAttributes())
                ->all()
        );

        return collect($rows)
            ->map(function (array $attributes): SeoRedirect {
                $redirect = new SeoRedirect;
                $redirect->setRawAttributes($attributes, true);
    ...
    protected function regexMatches(string $pattern, string $path, Request $request): bool
    {
        ...
        return (bool) preg_match($regex, $path)
            || (bool) preg_match($regex, '/'.$path)
            || (bool) preg_match($regex, $request->getRequestUri());
    }
