...
namespace Step2dev\LazySeoRedirect\Http\Middleware;
...
class HandleSeoRedirects
{
    ...
    {
        if (! config('lazy-seo-redirect.enabled', true)) {
            return $next($request);
        }
        ...
        $target = $this->targetUrl($this->resolvedTarget($redirect, $request), $request);

        if (! $this->safety->isAllowedTarget($target, $request)) {
            return $next($request);
        }
    ...
    {
        $allowedStatusCodes = config('lazy-seo-redirect.allowed_status_codes', [301, 302, 307, 308, 410]);
    ...
    {
        if (! config('lazy-seo-redirect.wildcard_enabled', true) && ! config('lazy-seo-redirect.regex_enabled', true)) {
            return null;
        }
    ...
    {
        $cacheSeconds = (int) config('lazy-seo-redirect.cache_seconds', 60);
        ...
        return Cache::remember('lazy-seo-redirect.patterns', $cacheSeconds, $callback);
    }
    ...
    protected function patternMatches(SeoRedirect $redirect, Request $request): bool
    {
        ...
        if (! $redirect->is_regex && config('lazy-seo-redirect.wildcard_enabled', true)) {
    ...
    {
        if (! config('lazy-seo-redirect.preserve_query', true) || $request->getQueryString() === null) {
            return $target;
        }
