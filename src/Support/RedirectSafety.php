...
class RedirectSafety
{
    public function isAllowedTarget(string $target, Request $request): bool
    {
        ...
        $allowedHosts = array_map('strtolower', config('lazy-seo-redirect.security.allowed_hosts', []));

        if (in_array(strtolower($host), $allowedHosts, true)) {
            return true;
        }

        return (bool) config('lazy-seo-redirect.security.allow_external_destinations', false)
            && $allowedHosts === [];
    }
